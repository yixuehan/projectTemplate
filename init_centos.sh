...
yum install llvm-toolset-7-cmake -y
...
{
    echo 编译grpc...
    yum install build-essential autoconf libtool pkg-config -y
    yum install libgflags-dev libgtest-dev -y
    yum install libc++-dev -y
    cd $shellpath/grpc
